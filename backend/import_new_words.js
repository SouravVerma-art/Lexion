...
async function importWords() {
  try {
    ...
    // 3. Filter words starting from "Swarm" (index 351, line 38,369)
    const swarmIndex = words.findIndex(w => w.word.toLowerCase() === 'swarm');
    if (swarmIndex === -1) {
      throw new Error('Could not find starting word "Swarm" in data.json.');
    }
    console.log(`Starting import from word "Swarm" (index ${swarmIndex}, line ~38,369).`);
    const wordsToProcess = words.slice(swarmIndex);
    console.log(`Found ${wordsToProcess.length} entries to process.`);
    ...
    for (let i = 0; i < wordsToProcess.length; i++) {
      ...
      if (!item.word || !item.definition || !item.partOfSpeech) {
        console.warn(`[Warning] Skipping invalid entry at index ${swarmIndex + i}:`, item);
        errorCount++;
        continue;
      }
